...
首先从 [Arch Linux 官网](https://archlinux.org/download)下载最新版 Arch Linux 镜像。
往下翻找到 China，选择一个镜像源（比如中科大 ustc.edu.cn）：
...
| VirtIO 块设备（虚拟硬盘）         | /dev/vd[a-z]         |

对于 SATA 硬盘/U 盘/VirtIO 块设备来说，系统采用 a~z 来代表 26 块不同的硬盘（从 a 开始分配）。对于 NVMe 硬盘来说，第一个数字表示控制器序号（从 0 开始），第二个数字表示命名空间（namespace）序号（从 1 开始）。但是对于消费级 NVMe 硬盘而言，一块硬盘一般只有一个命名空间，因此我们一般见到的是 ```/dev/nvme[0-9]n1```。

- 有关设备文件的详细介绍，请参阅 [Device file](https://wiki.archlinux.org/title/Device_file).
...
（摘自 [Red Hat Recommended System Swap Space](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/managing_storage_devices/getting-started-with-swap#recommended-system-swap-space)）
...
- GRUB 的很多选项都是可定制的。关于如何修改 GRUB 的启动顺序、主题、内核启动参数等问题，请参阅 [GRUB](https://wiki.archlinux.org/title/GRUB)。
## SSD TRIM
...
## 可选：启用 32 位应用支持（若不需要安装 Steam 则无需开启）
...
- 此处使用了新一代声音服务器 PipeWire，延迟比 PulseAudio 更低。有关 PipeWire 的其他信息，请参阅 [PipeWire](https://wiki.archlinux.org/title/PipeWire)。
...
**注意**：NVIDIA 590 驱动已停止支持 Pascal 及更早架构。

对于 Turing 及更新架构的 NVIDIA 显卡，执行下面命令安装驱动：

```bash
pacman -S nvidia-open-dkms nvidia-settings nvidia-utils
```

对于 Maxwell、Pascal 和 Volta 架构的 NVIDIA 显卡，需要从 AUR 安装 [nvidia-580xx-dkms](https://aur.archlinux.org/packages/nvidia-580xx-dkms)：

```bash
pacman -S git
git clone https://aur.archlinux.org/nvidia-580xx-utils.git
makepkg -D nvidia-580xx-utils -si
```
